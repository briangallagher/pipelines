...
def detect_model_bias(
  ...
  Args:
      ...
      predictions_format: The file format for the batch prediction results. `jsonl` is the only currently allow format.
      ...
      bias_configs: A list of `google.cloud.aiplatform_v1beta1.types.ModelEvaluation.BiasConfig`. When provided, compute model bias metrics for each defined slice. Below is an example of how to format this input.
        ...
        5: Finally, pass bias_configs as an parameter for this component. `DetectModelBiasOp(bias_configs=bias_configs)`

      thresholds: A list of float values to be used as prediction decision thresholds.
  ...
  Returns:
      bias_model_metrics: Artifact tracking the model bias detection output.
