...
def bigquery_drop_model_job(
  ...
  Args:
      location: Location of the job to drop the BigQuery model. If not set, default to `US` multi-region.  For more details, see https://cloud.google.com/bigquery/docs/locations#specifying_your_location
      model: BigQuery ML model to drop.
      ...
      project: Project to run BigQuery model drop job. Defaults to the project in which the PipelineJob is run.
  ...
  return ContainerSpec(
      ...
      command=[
          ...
          '-u',
          '-m',
          'google_cloud_pipeline_components.container.v1.bigquery.drop_model.launcher',
      ],
      args=[
          '--type',
